...
export function Project() {
    ...
    const inProgresIcon = () => {
        return (
            <span className="flex items-center justify-center mb-4 bg-gray-500 text-white md:py-2 px-2 md:px-3 rounded md:text-xs font-bold">IN PROGRESS</span>
        );
    };
                    ...
                    <div className="flex justify-center w-full gap-5">
                        ...
                        {projects.map((project, index) => (
                            <div key={index} className="w-xs md:w-sm border-2 rounded-2xl border-solid border-(--gray-900) p-5 hover:bg-(--pink-100)">
                                ...
                                {project.app ?
                                    <a href={project.app} className="flex items-center justify-center gap-1 mb-4 bg-gray-900 text-white md:py-2 px-2 md:px-3 rounded md:text-xs font-medium" target="_blank" rel="noopener noreferrer">
                                        <span>App</span>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-arrow-up-right h-4 w-4" aria-hidden="true"><path d="M7 7h10v10"></path><path d="M7 17 17 7"></path></svg>
                                    </a> 
                                    : ""
                                }
                                </div>
                                </div>
                                {project.live ? inProgresIcon() : ""}
                                <p>{project.details}</p>
